...
void GouraudScanConvertTriangle(GouraudVertex P1, GouraudVertex P2, GouraudVertex P3)
{
	...
	if  (P2.y > P1.y)
	{
		...
		Start.di = (P2.i - P1.i) / (float)(P2.y - P1.y);
	}
	...
	if  (P3.y > P1.y)
	{
		...
		Ende.di = (P3.i - P1.i) / (float)(P3.y - P1.y);
	}

	//Kommentar entfernen
	// Fuelle Sub-Dreick zwischen P1.y und P2.y 
	for (ScanLine = P1.y; ScanLine <P2.y; ScanLine++) {
	...
	if  (P3.y > P2.y)
	{
		...
		Start.x = (float)P2.x;
		Start.di = (P3.i - P2.i) / (float)(P3.y - P2.y);
		Start.i = P2.i;

		//Kommentar entfernen
		for (ScanLine = P2.y; ScanLine<P3.y; ScanLine++) {
			// Fuelle ScanLine von Start.x bis Ende.x
			GouraudDrawScanLine(Start.x, Ende.x, ScanLine, Start.i, Ende.i);
